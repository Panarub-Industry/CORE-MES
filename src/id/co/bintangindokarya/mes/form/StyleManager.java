...
public class StyleManager {
    ...
    public static String getZKResponsiveTableContainer() {
        return combine(
            ...
            BREAKPOINT_LAPTOP + " { max-height: 350px; }",
            BREAKPOINT_TABLET + " { max-height: 300px; }",
            BREAKPOINT_MOBILE + " { max-height: 250px; }"
        );
    }
    ...
    public static String getPOComboboxImproved() {
        return combine(
                ...
                // Hover and focus states for better UX - FIXED: Remove outline
                ":hover { border-color: #9ca3af; background-color: #ffffff; }",
                ":focus { border-color: #6b7280; box-shadow: 0 0 0 3px rgba(107, 114, 128, 0.1); outline: none !important; }",
                // Additional visual feedback when clickable
                ":active { background-color: #e9ecef; }",
                // Ensure proper event handling - prevent button interference
    ...
    public static String getNorthPanelSize() {
        return "65px"; // Restored to comfortable size for better visibility
    }
